   ...
   public function listar_filtro_ativo($A_USU_COD, $ORIGEM) {
      ...
      try{
         ...
         while ($row = $rs->fetch(PDO::FETCH_OBJ)) {
            ...
            if(!empty($row->FTR_ATENDENTE)){ $atendente = $row->FTR_ATENDENTE; } 
            if(!empty($row->FTR_PRIORIDADE)){ $prioridade = $row->FTR_PRIORIDADE; }
            if(!empty($row->FTR_CHASTATUS)){ $status = $row->FTR_CHASTATUS; }
            if(!empty($row->FTR_EMPSOLICITANTE)){ $empresa = $row->FTR_EMPSOLICITANTE; }
            if(!empty($row->FTR_SETOR)){ $setor = $row->FTR_SETOR; }
            ...
            $lista_filtro_relatorio_chamados[$i] = array(
               ...
               "FTR_DATADE" => $datade,
               "FTR_DATAATE" => $dataate,
               "FTR_ATENDENTE" => $atendente,
               "FTR_PRIORIDADE" => $prioridade,
   ...
   public function create() {
      try{
         $sql = "INSERT INTO `FTR`(
               ...
               `FTR_DATAATE`, `FTR_ATENDENTE`, 
               `FTR_PRIORIDADE`, `FTR_CHASTATUS`, 
               `FTR_EMPSOLICITANTE`, `FTR_SETOR`, 
               `FTR_DTCAD`, `FTR_HRCAD`
            ) VALUES (
               :FTR_USU, :FTR_ORIGEM, 
               :FTR_SITUACAO, :FTR_DATADE, 
         ...
         $sql->bindParam("FTR_ATENDENTE"   , $this->FTR_ATENDENTE   , PDO::PARAM_STR);
         $sql->bindParam("FTR_PRIORIDADE"         , $this->FTR_PRIORIDADE         , PDO::PARAM_STR);
         ...
         $sql->bindParam("FTR_SETOR"     , $this->FTR_SETOR     , PDO::PARAM_STR);
