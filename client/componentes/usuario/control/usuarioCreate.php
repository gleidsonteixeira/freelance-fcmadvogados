    ...
    if(isset($_POST["ACAO"])) {
        if(!empty($_POST["ACAO"]) && $_POST["ACAO"] == "CREATE") {
            if(isset($_POST["usuario-nome"]) && isset($_POST["usuario-email"]) && 
                ...
                isset($_POST['usuario-telefone']) && isset($_POST['usuario-menu'])
            ) {
                if(!empty($_POST["usuario-nome"]) && !empty($_POST["usuario-email"]) && 
                   !empty($_POST["usuario-senha"]) && !empty($_POST["usuario-cpf"]) && 
                   !empty($_POST["usuario-telefone"]) && !empty($_POST["usuario-menu"]) 
                ) {
                    ...
                    $usuario_empresa = $_SESSION['C_USU_EMP']; 
                    ...
                    $usuario->setUSU_USUCAD($_SESSION['C_USU_COD']);
                    ...
                    if($retorno['status'] == 0){
                        ...
                        $usuario_sa1->setSA1_USUCAD($_SESSION['C_USU_COD']);

                        $retorno_sa1 = $usuario_sa1->create();
                        if($retorno_sa1['status'] == 0){
                            $retorno_sa1_email = $usuario_sa1->enviar_email_cad_usuario($retorno_sa1['codigo']);
