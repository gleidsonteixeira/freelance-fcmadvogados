...
class CMS{             
   ...
   public function create() {
      try{
         ...
         $sql->bindParam("CMS_HRGASTA"    , $this->CMS_HRGASTA  , PDO::PARAM_STR);
   ...
   public function lista_respostas($CHA_COD, $COD_ULTIMO_CMS) {
      ...
      try{
         ...
         while ($row = $rs->fetch(PDO::FETCH_OBJ)) {
            ...
            $hrarray = explode(":", $row->CMS_HRGASTA);
            $CMS_HRGASTA = $hrarray[0]."h";
            if($hrarray[1] > 0){
               $CMS_HRGASTA .= $hrarray[1]."m";
            }else{
               $CMS_HRGASTA .= $hrarray[1];
            }
