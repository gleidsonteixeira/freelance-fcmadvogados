...
class CHA{         
   ...
   public function buscar_dados($CHA_COD) {
      ...
      try{
         ...
         (SELECT SA1.SA1_NOME FROM SA1 WHERE SA1.SA1_USU = CHA.CHA_ATENDENTE) AS CHA_NATENDENTE,
         (SELECT SA1.SA1_EMAIL FROM SA1 WHERE SA1.SA1_USU = CHA.CHA_ATENDENTE) AS SA1_EMAIL,
         (SELECT SA1.SA1_EMAIL FROM SA1 WHERE SA1.SA1_USU = CHA.CHA_SOLICITANTE) AS SA1_EMAILSOLICITANTE,
         ...
         $lista_chamados = array(
            ...
            "CHA_NATENDENTE" => utf8_encode($row->CHA_NATENDENTE),
            "CHA_SA1_EMAIL" => $row->SA1_EMAIL,
            "CHA_SA1_EMAILSOLICITANTE" => $row->SA1_EMAILSOLICITANTE,
            "CHA_ATENDENTE" => $row->CHA_ATENDENTE,
            "CHA_AVALIACAO" => $row->CHA_AVALIACAO,
   ...
   public function listar_chamados_quantidade($C_USU_COD) {
      $lista_chamados = array(); 
      try{
         $sql = "SELECT 
         (SELECT COUNT(CHA_COD) FROM CHA WHERE (CHA_STATUS = 1 OR CHA_STATUS = 2 OR CHA_STATUS = 3) AND CHA_SOLICITANTE = $C_USU_COD) AS CHA_ABERTO, 
         (SELECT COUNT(CHA_COD) FROM CHA WHERE CHA_STATUS = 4 AND CHA_SOLICITANTE = $C_USU_COD) AS CHA_FINALIZADO;";
         $rs = $this->con->prepare($sql);
         $rs->execute();
   ...
   public function verificar_se_existe_atendente() {
      try{
         $sql = "SELECT * FROM CHA WHERE CHA_COD = ".$this->CHA_COD;
         $rs = $this->con->prepare($sql);
         $rs->execute();
         $row = $rs->fetch(PDO::FETCH_OBJ);

         if(!empty($row->CHA_ATENDENTE)){
            return true;
         }
         return false;
      } catch (PDOException $e){
   ...
   public function listar_chamados_ativos($LIMIT, $STATUS, $C_USU_COD) {
      $lista_chamados = array(); 
      try{
         $sql_status = "";
         if($STATUS == 1 || $STATUS == 2 || $STATUS == 3){
            $sql_status = "(CHA_STATUS = 1 OR CHA_STATUS = 2 OR CHA_STATUS = 3)";
         }else{
            $sql_status = "CHA_STATUS = $STATUS";  
         }

         $sql = "SELECT CHA.*, 
         (SELECT STO.STO_NOME FROM STO WHERE STO.STO_COD = CHA.CHA_STO) AS CHA_NSTO, 
         (SELECT SA1.SA1_NOME FROM SA1 WHERE SA1.SA1_USU = CHA.CHA_ATENDENTE) AS CHA_NUSU,
         (SELECT PRI.PRI_NOME FROM PRI WHERE PRI.PRI_COD = CHA.CHA_PRI) AS CHA_NPRI
         FROM CHA WHERE $sql_status AND CHA_SOLICITANTE = $C_USU_COD ORDER BY CHA_COD DESC $LIMIT";
         $rs = $this->con->prepare($sql);
         $rs->execute();
         ...
         while ($row = $rs->fetch(PDO::FETCH_OBJ)) {
            $lista_chamados[$i] = array(
               ...
               "CHA_HREMISSAO" => date("H:i:s",strtotime($row->CHA_HREMISSAO)),
               "CHA_STO" => $row->CHA_STO,
               "CHA_SOLICITANTE" => $row->CHA_SOLICITANTE,
               "CHA_PRI" => $row->CHA_PRI,
               "CHA_NSTO" => utf8_encode($row->CHA_NSTO),
               "CHA_NUSU" => utf8_encode($row->CHA_NUSU),
               "CHA_NPRI" => utf8_encode($row->CHA_NPRI),
               "CHA_ATENDENTE" => $row->CHA_ATENDENTE
            );
            $i++;
   ...
   public function listar_chamados_pag_prox($LIMIT, $CHA_COD, $STATUS, $C_USU_COD) {
      $lista_chamados = array(); 
      try{
         $sql_status = "";
         if($STATUS == 1 || $STATUS == 2 || $STATUS == 3){
         ...
         (SELECT SA1.SA1_NOME FROM SA1 WHERE SA1.SA1_USU = CHA.CHA_ATENDENTE) AS CHA_NUSU,
         (SELECT PRI.PRI_NOME FROM PRI WHERE PRI.PRI_COD = CHA.CHA_PRI) AS CHA_NPRI
         FROM CHA WHERE $sql_status AND CHA_SOLICITANTE = $C_USU_COD AND CHA_COD < $CHA_COD ORDER BY CHA_COD DESC $LIMIT";
         $rs = $this->con->prepare($sql);
         $rs->execute();
   ...
   public function listar_chamados_pag_ante($LIMIT, $CHA_COD, $STATUS, $C_USU_COD) {
      $lista_chamados = array(); 
      ...
      try{
         ...
         (SELECT SA1.SA1_NOME FROM SA1 WHERE SA1.SA1_USU = CHA.CHA_ATENDENTE) AS CHA_NUSU,
         (SELECT PRI.PRI_NOME FROM PRI WHERE PRI.PRI_COD = CHA.CHA_PRI) AS CHA_NPRI
         FROM CHA WHERE $sql_status AND CHA_SOLICITANTE = $C_USU_COD  AND CHA_COD > $CHA_COD $LIMIT";
         $rs = $this->con->prepare($sql);
         $rs->execute();
         ...
         while ($o < count($lista_chamados)) {
            $lista_chamados_order[$o] = array(
               ...
               "CHA_SOLICITANTE" => $lista_chamados[$i]['CHA_SOLICITANTE'],
               "CHA_PRI" => $lista_chamados[$i]['CHA_PRI'],
               "CHA_NSTO" => $lista_chamados[$i]['CHA_NSTO'],
               "CHA_NUSU" => $lista_chamados[$i]['CHA_NUSU'],
   ...
   public function create() {
      try{         
         ...
         if($sql->execute()){
            ...
            return array("status" => 0, "mensagem" => "Cadastro de Chamado realizado com Sucesso. Código do Chamado: ".$this->CHA_COD, "codigo" => $this->CHA_COD, "cod_solicitante" => $this->CHA_SOLICITANTE);
         }else{
   ...
   public function recusar_finalizacao(){
      try{
         $sql = "UPDATE `CHA` SET
         ...
         if($sql->execute()){
            ...
            return array("status" => 0, "mensagem" => "Recusa de Finalização feita com sucesso.", "codigo" => $this->CHA_COD, "CHA_ATENDENTE" => $retorno["CHA_ATENDENTE"]);
         }else{
   ...
   public function update_finalizacao(){
      try{
         $sql = "UPDATE `CHA` SET
               `CHA_STATUS` = :CHA_STATUS,  CHA_DTFINAL = :CHA_DTFINAL, CHA_HRFINAL = :CHA_HRFINAL
               WHERE `CHA_COD` = :CHA_COD";

         $sql = $this->con->prepare($sql);
         $sql->bindParam("CHA_STATUS"     , $this->CHA_STATUS     , PDO::PARAM_STR);
         $sql->bindParam("CHA_DTFINAL"     , $this->CHA_DTFINAL     , PDO::PARAM_STR);
         $sql->bindParam("CHA_HRFINAL"     , $this->CHA_HRFINAL     , PDO::PARAM_STR);
         $sql->bindParam("CHA_COD"           , $this->CHA_COD           , PDO::PARAM_STR);

         if($sql->execute()){
            $retorno = $this->buscar_dados($this->CHA_COD);
            return array("status" => 0, "mensagem" => "Finalização feita com sucesso.", "codigo" => $this->CHA_COD, "CHA_ATENDENTE" => $retorno["CHA_ATENDENTE"]);
         }else{
   ...
   public function realizar_avaliacao(){
      try{
         $sql = "UPDATE `CHA` SET
               `CHA_AVALIACAO` = :CHA_AVALIACAO,  CHA_DTAVALIACAO = :CHA_DTAVALIACAO, CHA_HRAVALIACAO = :CHA_HRAVALIACAO, CHA_AVALIAOPINIAO = :CHA_AVALIAOPINIAO
               WHERE `CHA_COD` = :CHA_COD";

         $sql = $this->con->prepare($sql);
         $sql->bindParam("CHA_AVALIACAO"     , $this->CHA_AVALIACAO     , PDO::PARAM_STR);
         $sql->bindParam("CHA_DTAVALIACAO"   , $this->CHA_DTAVALIACAO   , PDO::PARAM_STR);
         $sql->bindParam("CHA_HRAVALIACAO"   , $this->CHA_HRAVALIACAO   , PDO::PARAM_STR);
         $sql->bindParam("CHA_AVALIAOPINIAO" , $this->CHA_AVALIAOPINIAO , PDO::PARAM_STR);
         $sql->bindParam("CHA_COD"           , $this->CHA_COD           , PDO::PARAM_STR);

         if($sql->execute()){
            $retorno = $this->buscar_dados($this->CHA_COD);
            return array("status" => 0, "mensagem" => "Avaliação feita com sucesso.", "codigo" => $this->CHA_COD);
         }else{
            return array("status" => 1, "mensagem" => "Erro ao Avaliar.");
         }
      } catch (PDOException $e){
         return array("status" => 2, "mensagem" => "Erro ao Avaliar.");
      }  
   }
         ...
         $html = '<!DOCTYPE html>
            ...
            <body style="background-color: #efefef;padding: 0;margin: 0;">
                ...
                <div style="width: 500px;margin: 0 auto;position: relative;background-color: white;border-radius: 30px;margin-top: -75px;padding: 16px 36px;">
                    ...
                    <h5 style="font-family: \'Oswald\', sans-serif;font-size: 22px;color: #a933cb;margin-bottom: 16px;">Olá <span>'.$retorno["CHA_NATENDENTE"].'</span></h5>
   ...
   public function enviar_email_cadchamados($CHA_COD){
      try {
         $retorno = $this->buscar_dados($CHA_COD);
         $html = '<!DOCTYPE html>
                     <html lang="en">
         ...
         $email_remetente = "suporte@objetiveti.com.br";
         $email_destinatario = trim ($retorno['CHA_SA1_EMAILSOLICITANTE']).';'.$email_remetente.';'.trim ($retorno['CHA_COPIA']);
