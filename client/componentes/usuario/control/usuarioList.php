<?php
    ...
    if(isset($_POST["ACAO"])) {
        if(!empty($_POST["ACAO"]) && $_POST["ACAO"] == "LISTAR USUARIO") {
            $usuario = new SA1();
            $lista_usuario = $usuario->listar_usuario_ativos("LIMIT 10", $_SESSION['C_USU_EMP'], $_SESSION['C_USU_COD']);
            ...
            if(isset($_POST["SA1_COD"])){
                $usuario = new SA1();
                $lista_usuario = $usuario->listar_usuario_pag_prox("LIMIT 10", $_POST["SA1_COD"], $_SESSION['C_USU_EMP'], $_SESSION['C_USU_COD']);
            ...
            if(isset($_POST["SA1_COD"])){
                $usuario = new SA1();
                $lista_usuario = $usuario->listar_usuario_pag_ante("LIMIT 10", $_POST["SA1_COD"], $_SESSION['C_USU_EMP'], $_SESSION['C_USU_COD']);
            ...
            echo json_encode($retorno);
        }else if(!empty($_POST["ACAO"]) && $_POST["ACAO"] == "LISTAR MENU") {
            $menu = new MNU();
            $lista_menu = $menu->listar_menu_ativo_tipo($_SESSION['C_USU_COD']);

            $retorno = array("status" => 0, "mensagem" => "Listagem realizada com sucesso", "lista_menu" => $lista_menu);
            echo json_encode($retorno);
